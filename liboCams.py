...
class v4l2_util():
  def __init__(self, devpath, verbose=False):
    ...
    try:
      self.vd = os.open(devpath, os.O_RDWR)
  ...
  def EnumerateControls(self):
    ...
    for i in range(20): #while True:
      ...
      if qc.type == v4l2.V4L2_CTRL_TYPE_MENU :
        ...
        querymenu.id = qc.id
        querymenu.index = qc.minimum
        menu = {}
        while querymenu.index <= qc.maximum:
          try:
            xioctl(self.vd, v4l2.VIDIOC_QUERYMENU, querymenu)
          except:
            break;
  ...
  def GetCurrentFormat(self):
      ...
      self.shape = (form.fmt.pix.height, form.fmt.pix.width)  # 셀프에 할당
      ...
      try:
          ...
          print(f"Failed to get stream parameters: {e}")
          raise
  ...
  def Start(self, buff_num):
    ...
    for i in range(n_buffers):
      ...
      self.buffers[i].start = mmap.mmap(self.vd, buf.length)
#,
...
class oCams():
  def __init__(self, dev, verbose=0):
    ...
    if self.cam.card == b'oCamS-1CGN-U':
      self.mode = 0
      ...
      self.mode = 2
    elif self.cam.card == b'oCamStereo-K':
      self.mode = 3
    else:
      self.mode = 1
...
def FindCamera(strCamera):
    try:
        # /dev/ 디렉토리에서 'video'로 시작하는 모든 파일을 탐색
        for path in os.listdir('/dev/'):
            if path.startswith('video'):  # video0, video1 등
                devpath = os.path.join('/dev/', path)
                print(f"Checking: {devpath}")  # 디버깅용 출력

                try:
                    # 장치를 열고 v4l2_capability를 통해 장치 속성 확인
                    fd = os.open(devpath, os.O_RDWR)
                    cp = v4l2.v4l2_capability()
                    fcntl.ioctl(fd, v4l2.VIDIOC_QUERYCAP, cp)

                    # 카드 이름을 확인하여 일치하는지 확인
                    if strCamera.encode() in cp.card:
                        print(f"Found camera: {devpath} with name: {cp.card.decode()}")
                        os.close(fd)
                        return devpath
                    os.close(fd)

                except Exception as e:
                    print(f"Failed to check {devpath}: {e}")

        print(f"No camera found matching: {strCamera}")
    except OSError as Err:
        print('Device Not Found!! Check Connection', Err)
    return None
